...
然而我们会发现一个问题，通过辐射通量来描述一个光源的明亮程度并不那么合理。例如我们同时买了一个40W的白炽灯和一个40W的日光色荧光灯，很明显我们会观察到荧光灯比白炽灯更明亮，但又并非两盏灯功率不一样。出现这种现象的原因有两点。第一，人的眼睛之所以能看到七种颜色的光，是因为这些光线都比较容易在水里传播，地球上的生命源于海洋也只需要看到这些光线即可生存，所以人类值看到这个波段的光，超出可见光波段的光将会以红外线、紫外线灯看不见的能量形式传播；第二，人眼对不同波段的光视见率不同，所以对于可见光来说，颜色不同人类也会觉得明亮程度不同。因此通常用光通量来描述人眼感受到的光的辐射功率。
...
光通量只能描述光源做功的效率，然而光线从光源发射出来并不是均匀的，在中国有一个单位来描述光线，也就是通常所说的一缕光线。那么如何来描述“一缕光线”的强弱呢？我们引入光强度这个物理量来描述光线的强弱。
...
现实生活中，人眼要能感觉到光线的存在由两种方式。一种方式就是直接盯着光源看，会看到一片明亮；另一种方式，光线需要照射到某个平面，通过平面把光线反射到人眼睛里，人眼才能感受到明亮。由此可知，人眼感觉到光线的多少还与光线照射的面积大小有关，于是我们使用光照度来描述。
...
设微表面面积为dS，光的方向和微表面方向夹角为θ，那么微表面垂直于光传播方向的投影$$dS' = dS · cosθ$$，则光源在该方向上的亮度为$$L = \frac{dI}{dS'} = \frac{dI}{dS · cosθ}$$，又由于$$dI = \frac{dΦ}{dΩ}$$，则光亮度又可以写作$$L = \frac{dΦ}{dΩ·dS·cosθ}$$。
...
由上图可知，光通量是描述光源自身亮度的，光强度是描述光线强度的，光通量是描述平面接收到的光能的多少，光亮度是描述眼睛感受到的光能的多少。
...
通常我们表示立体角方向习惯用ω，而表示立体角时候才用Ω，所以光亮度可以如下表示
...
这里的Ω表示半球（半球下的光被物体本身遮挡，对照明没有贡献）的立体角度，也就是在半球上，对来自不同方向的光线做光亮度积分，最终得到这一点上的光照度，这就是光照度和光亮度之间的关系。
...
从光源照射到平面上的光叫做入射光（incident light），照在平面上的光的总量可以用**光照度**来度量；从平面反射到眼球中的光叫做发射光（reflection light），反射出光的总量可以用**光亮度**来度量。反射系数指的就是反射光的强度与入射光的强度之比值。

> 例如，一般白纸大约吸收入射光量的20%，反射入射光量的80%，但是黑纸只反射入射光量的3%。

设反射系数为$$f_r$$，射入射光方向为$$ω_i$$，反射光方向为$$ω_r$$，那么微表面上的反射光亮度为$$dL_r(ω_r)$$，微表面上入射光照度为$$dE_i(ω_i)$$，反射系数我们可以如下表示
