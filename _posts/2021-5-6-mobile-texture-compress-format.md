...
那么有没有什么GPU能识别的纹理格式是像png、jpg这种经过压缩而又能最大程度保持原图效果的格式呢？答案是有的。常用的纹理压缩格式有ETC1、ETC2、PVRTC、ASTC等。这些纹理压缩格式的像素深度都远小于32bpp。例如ETC1的像素深度为4bpp，那么ETC1相对于ARGB32的压缩率也就是12.5%。换句话说，如果一个游戏使用了ARGB32这种未压缩的纹理格式，其贴图内存为800MB，如果将其纹理格式替换为ETC1，那么其贴图内存为100MB，且图片质量不会下降很多。
